 ...
 */
async function fetchReviewDetails(reviewId: string): Promise<Review | null> {
 ...
 */
function populateGameSelector(games: Game[]): void {
    const selector = document.getElementById('game') as HTMLSelectElement;
    if (!selector) return;
 ...
 */
async function handleFormSubmit(event: Event, reviewId: string | null): Promise<void> {
    ...
    const gameId = isUpdateMode
        ? (document.getElementById('game-display') as HTMLElement).dataset.gameId
        : (form.elements.namedItem('game') as HTMLSelectElement).value;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const formLegend = document.getElementById('form-legend') as HTMLElement;
    const submitButton = document.getElementById('submit-button') as HTMLButtonElement;
    const gameSelectorContainer = document.getElementById('game-selector-container') as HTMLElement;
    const gameDisplayContainer = document.getElementById('game-display-container') as HTMLElement;
    ...
    if (isUpdateMode) {
        ...
        if (review) {
            (document.getElementById('rating') as HTMLInputElement).value = review.rating.toString();
            ...
            gameDisplay.dataset.gameId = (review.game as any).id; // Assuming review.game has an id
        } else {
            (document.getElementById('review-form-container') as HTMLElement).innerHTML = '<p>Erro: Não foi possível carregar a avaliação para edição.</p>';
        }
    ...
    form?.addEventListener('submit', (event) => handleFormSubmit(event, reviewId));
});
